...
def _label(value: object) -> str:
    """Display label for a metadata value. Missing is its own category rather
    than a crash or a silent bucket with the empty string."""
...
class AtlasStore:
    ...
    def _build_coloring(self, field: str) -> tuple[np.ndarray, list[str]]:
        ...
        tally: dict[str, int] = {}
        for v in values:
            tally[_label(v)] = tally.get(_label(v), 0) + 1
        unique = np.array(list(tally), dtype=object)
        counts = np.array(list(tally.values()), dtype=np.int64)
        ...
        other = len(legend)
        indices = np.fromiter(
            (lookup.get(_label(v), other) for v in values),
            dtype=np.uint8,
            count=len(values),
        )
