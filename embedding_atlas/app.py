...
def point(index: int) -> dict:
    ...
    detail["video_url"] = oci_s3.presign_get(
        detail["source_media_uri"], _STATE["s3"], _presign_ttl_s()
    )
    return detail
...
def neighbors(index: int, k: int = 24) -> dict:
    ...
    for item in results:
        item["video_url"] = oci_s3.presign_get(
            item["source_media_uri"], _STATE["s3"], _presign_ttl_s()
        )
