...
# ============================ embedding atlas ==============================
# The same corpus as a 2D map instead of a ranked list: "what is in here" rather
# than "find me this". Served by THIS app rather than a second Cloud Run
# service -- it reads a precomputed ~63MB artifact, so it costs a rounding error
# next to the resident screen, and running it separately meant a second
# deployment, a second set of secrets and a duplicated oci_s3.
#
# Loaded lazily and never at startup: nobody who only searches should pay for
# it, and a failure here must not touch retrieval. NLS_ATLAS_URI unset means the
# feature is simply absent (404), not broken.
_ATLAS_URI = os.getenv("NLS_ATLAS_URI", "").strip()
_ATLAS: dict = {"store": None, "status": "idle", "error": ""}
_ATLAS_LOCK = threading.Lock()


def _atlas_load_worker() -> None:
    import atlas_store

    try:
        local = local_cache.cache_root() / "atlas" / _ATLAS_URI.rsplit("/", 1)[-1]
        if not local.exists():
            local.parent.mkdir(parents=True, exist_ok=True)
            LOGGER.info("atlas: downloading %s", _ATLAS_URI)
            oci_s3.download_object(_ATLAS_URI, local, _state["s3"])
        store = atlas_store.AtlasStore(local)
        with _ATLAS_LOCK:
            _ATLAS.update(store=store, status="ready", error="")
        LOGGER.info("atlas ready: %d points", store.count)
    except Exception as exc:  # noqa: BLE001 -- surfaced through the status field
        LOGGER.exception("atlas load failed")
        with _ATLAS_LOCK:
            _ATLAS.update(store=None, status="error", error=f"{type(exc).__name__}: {exc}")


def _atlas():
    """The loaded atlas, kicking the load on first use."""
    if not _ATLAS_URI:
        raise HTTPException(404, "atlas not configured (NLS_ATLAS_URI unset)")
    with _ATLAS_LOCK:
        store, status, error = _ATLAS["store"], _ATLAS["status"], _ATLAS["error"]
        if store is None and status == "idle":
            _ATLAS["status"] = "loading"
            start = True
        else:
            start = False
    if start:
        threading.Thread(target=_atlas_load_worker, name="atlas-load", daemon=True).start()
    if store is not None:
        return store
    if status == "error":
        raise HTTPException(503, f"atlas unavailable: {error}")
    raise HTTPException(503, "atlas is loading; retry shortly")


def _atlas_video_url(uri: "str | None") -> str:
    """Presigned clip URL, or "" when the row has no media.

    A null source_media_uri is ordinary in this corpus. Presigning it raised
    inside parse_s3_uri, so one null row failed a whole neighbours or lasso
    response rather than that one card.
    """
    if not uri:
        return ""
    try:
        return oci_s3.presign_get(uri, _state["s3"], _state["cfg"].presign_ttl_s)
    except (ValueError, botocore.exceptions.ClientError) as exc:
        LOGGER.warning("atlas: could not presign %s: %s", uri, exc)
        return ""
...
def atlas_points() -> Response:
    return Response(content=_atlas().positions, media_type="application/octet-stream")


@app.get("/api/atlas/coloring/{field}")
def atlas_coloring(field: str) -> Response:
    atlas = _atlas()
    if field not in atlas.color_fields:
        raise HTTPException(404, f"unknown color field {field!r}")
    indices, _legend = atlas.coloring(field)
    return Response(content=indices, media_type="application/octet-stream")


@app.get("/api/atlas/legend/{field}")
def atlas_legend(field: str) -> dict:
    atlas = _atlas()
    if field not in atlas.color_fields:
        raise HTTPException(404, f"unknown color field {field!r}")
    _indices, legend = atlas.coloring(field)
    return {"field": field, "legend": legend}


@app.get("/api/atlas/point/{index}")
def atlas_point(index: int) -> dict:
    atlas = _atlas()
    if not 0 <= index < atlas.count:
        raise HTTPException(404, "index out of range")
    detail = atlas.detail(index)
    detail["video_url"] = _atlas_video_url(detail.get("source_media_uri"))
    return detail


@app.get("/api/atlas/neighbors/{index}")
def atlas_neighbors(index: int, k: int = 24) -> dict:
    atlas = _atlas()
    if not 0 <= index < atlas.count:
        raise HTTPException(404, "index out of range")
    results = atlas.neighbors(index, min(k, 96))
    for item in results:
        item["video_url"] = _atlas_video_url(item.get("source_media_uri"))
...
@app.post("/api/atlas/lasso")
def atlas_lasso(req: LassoRequest) -> dict:
    atlas = _atlas()
    if len(req.polygon) < 3:
        raise HTTPException(400, "polygon needs >= 3 vertices")
    items = []
    for index in atlas.lasso(req.polygon, min(req.limit, 96)):
        detail = atlas.detail(index)
        detail["video_url"] = _atlas_video_url(detail.get("source_media_uri"))
        items.append(detail)
